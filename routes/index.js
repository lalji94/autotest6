...
function postImageWidth(post_link,token,amzn_data,storeId,finalAmznData) {
  axios(post_link)
  // axios('https://www.amazon.in/dp/B07DJD1RTM')
      .then(response => {
		...
		connection.query(sqlss, function (err, rides) {
          ...
          if(siteheadidsdng && siteheading && sitestrckprice && sitestrckpricessds && savepercent ){
            telePost(token,siteheadidsdng,siteheading,sitestrckprice,sitestrckpricessds,savepercent,post_link,avilabilty,amzn_data)
         } else if(siteheadidsdng && siteheading && sitestrckpricessds && avilabilty ){
            telePosted(token,siteheadidsdng,siteheading,sitestrckpricessds,post_link,avilabilty,amzn_data)
        }else{
		    	let postedImg = "";
			    whatsapp_posts1(finalAmznData,postedImg,amzn_data[0].apiKey,amzn_data[0].phoneId,amzn_data[0].productId);
		      whatsapp_posts2(finalAmznData,postedImg,amzn_data[1].apiKey,amzn_data[1].phoneId,amzn_data[1].productId);
        }
		  }
		})
      })
    ...
    function telePost (token,post_img,post_title,post_regularPrice,post_sellPrice,savepercent,post_link,avilabilty,finalIdList) {
      var chatId = '@bestshoppingdeal00'; // <= replace with yours
    
      var html = '🛍 ' + post_title + '\n\n' +
        '🔗 ' + post_link + '\n' +
        '♨️ PRICE : ' + post_sellPrice + '\n' +
        '🚫 M.R.P. : ' + post_regularPrice + '\n' +
        '💰 SAVINGS : ' + savepercent + '\n' +
        '🙋 AVAILABILITY : ' + avilabilty + '\n' +
        '🚚 FREE Delivery\n';
      var buttons = [
        [
      ...
      if (html) {
        whatsapp_posts1(html,post_img,finalIdList[0].apiKey,finalIdList[0].phoneId,finalIdList[0].productId);
        whatsapp_posts2(html,post_img,finalIdList[1].apiKey,finalIdList[1].phoneId,finalIdList[1].productId);
      }
    }

    function telePosted (token,post_img,post_title,post_sellPrice,post_link,avilabilty,finalIdList) {
      var chatId = '@bestshoppingdeal00'; // <= replace with yours
  ...
  setInterval( function setup() {
      axios('https://t.me/s/linkconvertwatts')
        .then(response => {
          var html = response.data;
          var $ = cheerio.load(html);
          var matchObj = [];
                   ...
                   example(tagnot.replace(/&demoyou/g, '').replace(/%25/g,'%'));
                       ...
                       // if(array_length == 1){
						//	let finalIdListed = JSON.parse(ListflagData.array_data).user;
							//	postImageWidth(response.link,ListflagData.bestshopping_token,finalIdListed,nextId);
						//	}
                      }
